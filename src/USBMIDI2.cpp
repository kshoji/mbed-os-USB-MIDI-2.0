...
USBMIDI2::USBMIDI2(bool connect_blocking, uint16_t vendor_id, uint16_t product_id, uint16_t product_release)
...
USBMIDI2::USBMIDI2(USBPhy *phy, uint16_t vendor_id, uint16_t product_id, uint16_t product_release)
...
void USBMIDI2::wait_ready()
{
    _flags.wait_any(FLAG_CONNECT, osWaitForever, false);
}

// write plain MIDI2Message that will be converted to USBMidi event packet
bool USBMIDI2::write(MIDI2Message m)
{
    _write_mutex.lock();
    ...
    for (int p = 1; p < m.length; p += 3) {
        ...
        if (buf[0] == 0xF) {
            ...
            if ((m.length - p) > 3) {
                if (p == 1) {
                    // SysEx start
                    buf[1] = 0x10 | std::min((m.length - p), 3);
                } else {
                    // SysEx continue
                    buf[1] = 0x20 | std::min((m.length - p), 3);
                }
            } else {
                // SysEx end
                buf[1] = 0x30 | (m.length - p);
            }
            ...
            USBDevice::write_start(_bulk_in, buf, 8);
...
bool USBMIDI2::read(MIDI2Message *m)
{
    lock();
...
void USBMIDI2::attach(mbed::Callback<void()> callback)
{
    lock();
...
void USBMIDI2::callback_state_change(DeviceState new_state)
{
    assert_locked();
...
void USBMIDI2::callback_request(const setup_packet_t *setup)
{
    assert_locked();
...
void USBMIDI2::callback_request_xfer_done(const setup_packet_t *setup, bool aborted)
{
    assert_locked();
...
void USBMIDI2::callback_set_configuration(uint8_t configuration)
{
    assert_locked();
    ...
    endpoint_remove_all();
    endpoint_add(_bulk_in, MaxSize, USB_EP_TYPE_BULK, &USBMIDI2::_in_callback);
    endpoint_add(_bulk_out, MaxSize, USB_EP_TYPE_BULK, &USBMIDI2::_out_callback);
...
void USBMIDI2::callback_set_interface(uint16_t interface, uint8_t alternate)
{
    assert_locked();
...
const uint8_t *USBMIDI2::string_iinterface_desc()
{
...
const uint8_t *USBMIDI2::string_iproduct_desc()
{
...
const uint8_t *USBMIDI2::configuration_desc(uint8_t index)
{
    if (index != 0) {
        return NULL;
    }
    ...
    uint8_t config_descriptor_temp[] = {
        ...
        0x07, 0x24, 0x01, 0x00, 0x02/*MIDI 2.0*/, 0x41, 0x00,             // Class-Specific MS Interface Header Descriptor
...
bool USBMIDI2::_next_message()
{
    ...
    while (_bulk_buf_pos < _bulk_buf_size) {
        uint8_t data_read;
        bool data_end = true;
        switch (_bulk_buf[_bulk_buf_pos] & 0xf0) {
            case 0x00:
                data_read = 3;
                break;
            case 0x10:
